...
1. （片側ずつ両方とも）電源スイッチがオフの状態になっていることを確認し、USBケーブルを接続して、以下のいずれかの方法で `RPI-RP2` リムーバブルドライブをマウントさせます。
   ...
   ![RPI-RP2 drive](../images/firmware_drive.png)
1. （片側ずつ両方とも）マウントされた `RPI-RP2` ドライブ内にファームウェアのファイルをコピーして保存します。
   自動的にマウントが解除され、キーボードが再起動します。
   再起動したら、一度USBケーブルを抜いてください。
1. 単三電池をセットし、左、右の順にスイッチをオンにします。
1. 接続するPCでBluetooth接続をしてください。
うまくいかないときは、リセット用ファームウェアを左右両方に書き込んだ後、PCで保存した端末の情報を削除してから、最初の手順からやり直してください、
